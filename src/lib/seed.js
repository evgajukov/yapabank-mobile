export function seedCheck(seed) {
  console.log("seed check");
  if (seed == null || seed.trim().length == 0) return false;
  let seedList = seed.toLowerCase().split(/\s+/);
  for (let word of seedList) {
    // проверяем слова на наличие в словаре
    if (seedWordsList.indexOf(word) == -1) return false;
  }
  return true;
}

export function seedNormalize(seed) {
  console.log("seed normalize");
  if (seed == null) return null;
  const newSeed = seed.trim().toLowerCase();
  const newSeedList = newSeed.split(/\s+/);
  return newSeedList.join(" ");
}

export const seedWordsList = [
